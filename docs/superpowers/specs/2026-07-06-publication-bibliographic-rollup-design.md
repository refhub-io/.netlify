...
- **Vitest, mocking `supabase.rpc(...)`:** confirms the RPC is invoked with the correct `p_patch` when the body contains bibliographic fields; confirms it is *not* invoked for a tag-only or notes-only `PATCH` (no bibliographic fields present); confirms an RPC failure surfaces as the structured `502 publication_rollup_failed` response rather than a 200, and that the item is left unmodified from the caller's perspective (no `data` returned as if it succeeded).
